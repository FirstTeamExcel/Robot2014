...
#include "Math.h"
DriveToEncoderCommand::DriveToEncoderCommand(float distance, float power) {
...
void DriveToEncoderCommand::Execute() {
		    Robot::driveSubsystem->DriveStraight(_drivePower);
...
bool DriveToEncoderCommand::IsFinished() {
    ...
    float left = fabs(Robot::driveSubsystem->leftEncoder->GetDistance());
    float right = fabs(Robot::driveSubsystem->rightEncoder->GetDistance() );
	if ((right > _stopDistance) || (left > _stopDistance))
	{
		return true;
	}
	return false;
