...
#include "Commands/DriveSingleBall.h"
...
#include "Commands/SpinLongTruss.h"
...
OI::OI() {
	...
	collectorUp->WhenPressed(new CatchClose());
	...
	SmartDashboard::PutData("LoadIntoShooter", new LoadIntoShooter());
	SmartDashboard::PutData("BackwardAuto", new BackwardAuto());
	SmartDashboard::PutData("SpinLongTruss", new SpinLongTruss());
	SmartDashboard::PutData("DriveSingleBall", new DriveSingleBall());
	SmartDashboard::PutData("ManualTossTarget", new ManualTossTarget());
...
void OI::UpdateDefaults()
{
    static Command * shiftMode = (Command *) 0;
    static Command * driveMode = (Command *) 0;
    
    if(shiftMode != (Command *) shiftModeChooser->GetSelected())
    {
        shiftMode = (Command *) shiftModeChooser->GetSelected();
        Robot::shifters->SetDefaultCommand(shiftMode);
        
        Command *current = Robot::shifters->GetCurrentCommand();
        if (current != (Command *) 0)
            current->Cancel();
    }
    
    if(driveMode != (Command *) driveModeChooser->GetSelected())
    {
        driveMode = (Command *) driveModeChooser->GetSelected();
        Robot::driveSubsystem->SetDefaultCommand(driveMode);
        
        Command *current = Robot::driveSubsystem->GetCurrentCommand();
        if (current != (Command *) 0)
            current->Cancel();
    }
}
