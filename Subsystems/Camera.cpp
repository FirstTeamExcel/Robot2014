...
vector<ParticleAnalysisReport>* AnalyzeImage(ColorImage *image,
        BinaryImage *filteredImage, bool verticalOnly = false)
{
    BinaryImage *thresholdImage = image->ThresholdHSL(90, 115,30, 255, 70, 255);
    if ((thresholdImage == (void *) 0) || (thresholdImage->GetWidth() == 0) || (thresholdImage->GetHeight() == 0))
    {
        if (thresholdImage != (void *) 0)
            delete thresholdImage;
        
        return (vector<ParticleAnalysisReport>*) 0;
    }
...
//    thresholdImage->Write(fileName);
    ...
    BinaryImage *convexHullImage = hslImage->ConvexHull(false);
    if (verticalOnly == true)
        filteredImage = convexHullImage->ParticleFilter(vertCriteria, 2);
    else
        filteredImage = convexHullImage->ParticleFilter(criteria, 1);
