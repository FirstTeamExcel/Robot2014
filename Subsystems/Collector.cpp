...
{
	rollerMotor->Set(0.0);
	if ((GetFlipperState() == F_DOWN) && (_autoFlipTimer.HasPeriodPassed(_autoFlipTimeout)))
    {
	    SetFlipperPosition(false);
    }
}
...
void Collector::SetAutoFlipTimeout(float timeoutTime)
{
    _autoFlipTimer.Reset();
    if (timeoutTime > 0.0)
    {
        _autoFlipTimeout = timeoutTime;
